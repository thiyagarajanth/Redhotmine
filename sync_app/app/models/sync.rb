...
class Sync < ActiveRecord::Base
  ...
  def self.sync_sql
    ...
    user_data = "SELECT first_name, last_name, login_id, work_email, employee_no, is_active, modified_date, prev_emp_no, dep_name, loc_name, region_name, location_type FROM vw_employee where modified_date >= '#{@sync_time}' and employee_no != 0 and work_email != '' and login_id !='' order by modified_date asc"
    ...
    @user_info.each(:as => :hash) do |user|
      ...
      last_emp_code = "INSERT into user_official_infos (user_id, employee_id, department,location_name,region_name,location_type) values ((select id from users where login='#{user['login_id']}' limit 1),#{user['employee_no']},'#{user['dep_name']}','#{user['loc_name']}','#{user['region_name']}','#{user['location_type']}')"
    ...
    p " Total users in iNia after Sync : #{inia_users_count.count}"
    #================= user =============================
      rec.last_sync=Time.now
      rec.save
    #===================================================================================
