class WktimeController < ApplicationController
  ...
  def index
    ...
    if params[:project_id].present? && params[:user_id].to_i > 0
      ...
      time_entries = TimeEntry.where(spent_on: @startday..@endday,user_id: params[:user_id]).group('spent_on').sum('hours')
      ...
      user_list = total_users.flatten.uniq
      user_arr = total_users.uniq
      user_list.each do |user|
        ...
        if time_entries.present?
          ...
          @new_entries << {user.id => [time_entries, l3, l2,l1,r, l] }
        end
      end
...
# to change the status to locked user to unlock
          ...
          ids.each_with_index do |id, k|
            ...
            if disabled[k] == "false"
              if (check_time_log_entry((@startday + k).to_date,User.current).present? && check_time_log_entry((@startday + k).to_date,User.current) == true ) && (!id.blank? || !hours[j].blank?) || (get_entry_status((@startday + k).to_date).present? && get_entry_status((@startday + k).to_date)==true )
                ...
                #Added below line to avoid duplicate entries
                entry['flexioff_reason'] == "Others" ? entry['flexioff_reason'] = nil : entry['flexioff_reason']
  ...
  def findBySql(selectStr,sqlStr,wkSelectStr,wkSqlStr,from,to,ids)
    ...
    #commented below part is to avoid duplicate entries in home page
